  ...
  return (
    <div className="h-full w-full p-8 overflow-y-auto">
      ...
      <div className="flex justify-between items-center mb-6 h-10">
        <div className="flex bg-surface-hover border border-divider rounded-lg p-1">
          <button className="px-4 py-1.5 rounded-md bg-gray-800 text-primary text-xs font-medium">7 Days</button>
          <button className="px-4 py-1.5 rounded-md text-secondary hover:text-primary text-xs font-medium transition-colors">30 Days</button>
          <button className="px-4 py-1.5 rounded-md text-secondary hover:text-primary text-xs font-medium transition-colors">YTD</button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        {/* Main Chart Area */}
        <div className="lg:col-span-2 bg-surface border border-divider rounded-xl p-6 h-96 flex flex-col">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="text-lg font-medium text-primary">API Request Volume</h2>
              <p className="text-xs text-secondary">Total requests processed by the AI Engine</p>
            </div>
            <div className="text-right">
              <div className="text-2xl font-bold text-primary">124.5k</div>
          ...
          <div className="flex-1 flex items-end justify-between gap-2 relative">
            ...
            <div className="flex-1 flex items-end justify-between pl-8 h-full pb-6 border-b border-divider/50">
              {[40, 55, 35, 75, 60, 90, 85, 100, 70, 85, 60, 45, 80, 95].map((h, i) => (
                <div key={i} className="w-full mx-1 group relative flex justify-center h-full items-end">
                  <div 
                    ...
                    style={{ height: `${h}%` }}
                  ></div>
                  <div className="absolute -top-8 bg-gray-800 text-primary text-xs py-1 px-2 rounded opacity-0 group-hover:opacity-100 z-10">
                    {h * 200}
                  </div>
                </div>
              ))}
        ...
        <div className="lg:col-span-1 bg-surface border border-divider rounded-xl p-6 flex flex-col">
          <h2 className="text-lg font-medium text-primary mb-1">Request Sources</h2>
          <p className="text-xs text-secondary mb-8">Where traffic is originating from</p>
